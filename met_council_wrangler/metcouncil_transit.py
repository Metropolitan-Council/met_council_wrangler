...
from cube_wrangler.transit import StandardTransit, CubeTransformer
...
class CubeTransit(object):
    ...
    def __init__(
        ...
        if type(parameters) is dict:
            self.parameters = MetCouncil_Parameters(**parameters)
        elif isinstance(parameters, MetCouncil_Parameters):
            self.parameters = MetCouncil_Parameters(**parameters.__dict__)
        else:
...
def route_properties_gtfs_to_cube(
    ...
    trip_df["start_time_seconds"] = trip_df["start_time"].apply(_time_to_seconds)

    trip_df["tod_name"] = trip_df.start_time_seconds.apply(
        transit_net.time_to_cube_time_period
    )
